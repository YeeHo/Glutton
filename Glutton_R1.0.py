...
def GetCandidate(inputCS, ResNum, df, cutoff, weight):
    ...
    if abs(inputCS['HA'].iloc[ResNum]) < 500.0:
...
def AnglesLogCDF(data, NumofStructGen):
    """"Calculate CDF for data."""
    hist, bins = np.histogram(data, bins=36)
    hist[hist==0] = 1
    bin_midpoints = bins[:-1] + np.diff(bins)/2
    cdf = np.cumsum(np.log(hist))
    cdf = cdf / cdf[-1]
    values = np.random.rand(10*NumofStructGen)
    value_bins = np.searchsorted(cdf, values)
    angles = bin_midpoints[value_bins]
...
def GeneratePhiPsi(CSFile, databaseLevel, NumofStructGen, Weight, logCDF):
    ...
    for inum in range(iLen):
        ...
        if logCDF == True:
            Phi[inum,:] = AnglesLogCDF(dfx['PHI'].values, NumofStructGen)
            Psi[inum,:] = AnglesLogCDF(dfx['PSI'].values, NumofStructGen)
        else:
            Phi[inum,:] = AnglesCDF(dfx['PHI'].values, NumofStructGen)
            Psi[inum,:] = AnglesCDF(dfx['PSI'].values, NumofStructGen)

        ssType, ss = SSCount(dfx)
...
def SSCount(dfx):
    ...
    ss = [0.0, 0.0, 0.0]
    C = sum(dfx['SS'].str.count('C'))
    E = sum(dfx['SS'].str.count('E'))
    H = sum(dfx['SS'].str.count('H'))
    ss[0] = float(C)/(C+E+H)
    ss[1] = float(E)/(C+E+H)
    ss[2] = float(H)/(C+E+H)
    if max(C,E,H) == C:
...
def glutton():
    ...
    seq, Phi, Psi, NumofStructGen = GeneratePhiPsi(inputPara[0], int(inputPara[1]),
            int(inputPara[2]), float(inputPara[3]), bool(inputPara[4]))
